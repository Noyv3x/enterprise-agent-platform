...
class PlatformRuntimeManager:
    ...
    def prepare_hermes(self) -> RuntimeStatus:
        if not self.config.manage_hermes:
    ...
    def ensure_hermes_ready(self, *, wait: bool = True) -> RuntimeStatus:
        with self._lock:
            ...
            if self._hermes_process is None or self._hermes_process.poll() is not None:
                self._start_hermes()
            if wait and self.config.runtime_startup_wait_seconds > 0:
    ...
    def _wait_for_hermes(self) -> RuntimeStatus:
        deadline = time.monotonic() + self.config.runtime_startup_wait_seconds
    ...
    def _hermes_child_values(self) -> dict[str, str]:
        parsed = urllib.parse.urlparse(self.config.hermes_api_url)
        ...
        values = {
            ...
            "API_SERVER_MODEL_NAME": self.config.hermes_model,
    ...
    def _hermes_command(self) -> tuple[list[str], Path | None, str]:
        repo = self.config.hermes_repo
        if (repo / "hermes_cli" / "main.py").exists():
            return (
                [sys.executable, "-m", "hermes_cli.main", "gateway", "run", "--replace", "--quiet"],
                repo,
                f"local repository: {repo}",
            )
        hermes = shutil.which("hermes")
        if hermes:
            return ([hermes, "gateway", "run", "--replace", "--quiet"], None, f"CLI: {hermes}")
        return ([], None, f"Hermes repository/CLI not found at {repo}")
