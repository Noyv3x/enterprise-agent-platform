...
打开 `http://127.0.0.1:8765`。部署脚本会初始化 Cognee 和 Firecrawl submodule、创建根目录 `.venv`、安装 Python 平台、按 lockfile 构建 Node.js Agent 运行时，并启动本地 SearXNG 搜索服务及其他托管服务。支持 user-level systemd 时会安装 `enterprise-agent-platform.service`，否则以前台模式运行。
...
Agent 通过平台内部接口调用知识搜索和文档读取；网页搜索直接使用平台独立管理的本地 SearXNG，网页提取与抓取继续使用托管 Firecrawl，浏览器操作通过按 scope 隔离的 Camofox Profile。SearXNG 仅监听宿主机回环地址，不需要购买搜索 API，并与 Firecrawl 分开启动和检查健康状态。生成文件只允许从工作区和明确配置的媒体根目录回传。

如果改用另一个由运维自行管理、仍仅监听本机数值回环地址的 SearXNG，可通过环境变量关闭平台托管实例并调整地址；也可修改搜索总超时和冷启动等待时间：

```bash
export ENTERPRISE_MANAGE_SEARXNG=0
export ENTERPRISE_SEARXNG_API_URL='http://127.0.0.1:13003'
export ENTERPRISE_SEARXNG_TIMEOUT_SECONDS=20
export ENTERPRISE_SEARXNG_STARTUP_WAIT_SECONDS=300
```
