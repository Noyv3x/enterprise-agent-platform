...
import { CODEX_PROVIDER_ID, oauthProviderErrorText } from "../../../utils/oauth";
...
export function OAuthProviderCard({ provider }: { provider: OAuthProvider }) {
  ...
  const label = provider.id === CODEX_PROVIDER_ID ? t("admin.oauth.provider.codex") : provider.label || provider.id;
  return <ResourceRow title={label}
    status={<><StatusMark tone={provider.configured ? "success" : "neutral"}>{t(provider.configured ? "admin.oauth.verified" : "admin.oauth.unverified")}</StatusMark></>}
    ...
    {flow?.complete ? <Notice tone="success" title={t("admin.oauth.complete")} /> : flow?.kind === "device_code" ? <CodexOAuthFlow providerId={provider.id} flow={flow} /> : null}
  </ResourceRow>;
}
