...
func recoverCurrentCommand(arguments []string) error {
	...
	return recoverCurrentWithConfig(cfg, expectedSHA)
}
...
func recoverCurrentWithConfig(cfg config.Config, expectedSHA string) error {
...
func buildWithConfig(cfg config.Config) (*application, error) {
	...
	selfUpdater := &selfupdate.Manager{Profile: active, ConfigPath: cfg.ConfigPath, Root: filepath.Join(cfg.StateDir, "manager-binaries"), StatePath: filepath.Join(cfg.StateDir, "manager-binaries.json"), InstallPath: managerInstallPath(), SocketPath: cfg.SocketPath, ControlTokenFile: controlTokenPath, UnitName: profile.ManagerUnit, RunningVersion: version}
...
func serveCommandWithBuild(arguments []string, cfg config.Config, builder func(config.Config) (*application, error)) error {
	...
	startup, err := acquireServeStartupOwnership(cfg)
	if err != nil {
...
func acquireServeStartupOwnership(cfg config.Config) (*serveStartupAdmission, error) {
	active := identity.CompileTimeActiveProfile()
	profile := identity.TargetProfile()
	manager := &selfupdate.Manager{
		Profile:          active,
...
func selfUpdateWatchdogCommand(arguments []string) error {
	...
	configPath := set.String("config", "", "manager.toml path")
	if err := set.Parse(arguments); err != nil {
		return err
	}
	if *plan == "" {
	...
	return selfupdate.RunWatchdog(context.Background(), binding, *plan, nil)
}
