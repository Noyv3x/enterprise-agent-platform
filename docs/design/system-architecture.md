...
`PlatformRuntimeManager` 统一准备、启动、探测、重启和停止 Agent Runtime、Camoufox、SearXNG、Firecrawl，并准备 Cognee 环境。所有生成配置、安装产物、profile、日志和数据库位于平台数据目录；上游 submodule 工作树只作为锁定来源。详见[外部集成](integrations.md)。
