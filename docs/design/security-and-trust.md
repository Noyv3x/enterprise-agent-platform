...
当前 `2026082901` Platform 容器只允许主 entrypoint 与 Docker 固定健康探针的闭合 dropper 短暂以 root 运行；`serve`、`init-admin`、token 命令与健康探针必须在创建目录、读取 secret 或加载业务代码前立即清除附加组并降为部署 UID/GID，任意其它 root 命令一律拒绝。所有 root Python 都必须使用镜像内绝对解释器、isolated 模式和 root-owned 工作目录，不能从可写数据根或 Python 环境变量导入代码。

固定 `migrate` 还必须带镜像内固定的 `2026080801-to-2026082901` 兼容标记。entrypoint 先以部署 UID/GID 和 isolated Python 只读固定并检查 `platform.db` marker；只有精确来源 `2026080801` 才回到 root 执行旧 mountpoint 兼容，fresh/current 不执行，未知或不安全数据库失败关闭。兼容步骤自身不得打开数据库、读取 secret 或执行任意路径/命令。

兼容步骤只可在逐段 no-follow 固定且属于部署 UID/GID 的 `data/workspaces` 下处理规范命名的 workspace：部署用户自有的 `.agent-platform` 仅允许从精确 `0755` 收紧到 `0700`，内容不遍历、不改 owner；root 所有的目录必须精确为旧 Docker 生成的 `0755 .agent-platform/attachments` 两层空目录。为支持子后父的崩溃重试，root 父目录下唯一的空 `attachments` 也可已经属于部署 UID/GID 且为 `0755` 或 `0700`；除此之外的额外项、符号链接、挂载跨设备、owner/group/mode 差异或身份漂移均失败关闭。转换按子后父顺序非递归改为部署 UID/GID 与 `0700`，结束时重新固定并完整验证 data/workspaces 身份；随后 entrypoint 立即降权重新进入普通迁移，不能保留 root shell、能力或业务进程。fresh install 或已规范目录上的该固定步骤只能是无副作用 no-op；后续 baseline 必须随直接迁移消费者一并删除该标记与 root 兼容分支。
