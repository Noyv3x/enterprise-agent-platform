...
Platform 在构造任何 SQLite 连接、worker 或外部副作用组件前，必须从逐段 no-follow 打开并保留的数据根目录 fd 取得 profile 对应的实例锁。锁叶节只能是当前 Platform UID 拥有、单硬链接、`0600` 的普通文件，并以 `openat(O_CREAT | O_EXCL/O_RDWR | O_NOFOLLOW | O_CLOEXEC)` 安全创建或打开。只允许在 owner、类型、link count 和已打开 inode 都验明后通过 fd 收紧旧文件权限；符号链接、硬链接、owner/type 异常不得修复。取得非阻塞独占 flock 后、写入 PID 前后都要复核锁 fd、固定父目录项与当前规范数据根仍指向同一 inode；任一路径置换都释放锁并失败关闭。锁文件不在关闭时 unlink，锁 fd 与父目录 fd 贯穿 Platform 生命周期。source 和 target profile 虽使用不同固定叶名，必须共享这一安全契约。
...
完整 source-owner/Bridge 基线必须在任何配置默认值、Manager state、普通 operation、自更新恢复、control 或 Gateway 初始化之前构造身份 Router。Bridge 的普通 source 启动只注入编译期 source profile；Bridge target profile 只能由外置 handoff journal 在 owner、类型、权限、schema、事务、阶段、源/目标 binding、运行二进制与 stable inode 全部验证后注入。Cleanup 与后续 target-only binary 的编译期 canonical stage 则只注入 target profile，并在没有 handoff journal 的 fresh install/普通启动中使用它；Cleanup 已不存在可选择的 source profile。manifest、CLI、环境变量、品牌、目标目录、二进制 basename 或 Docker 对象不得成为 profile 选择信号。Bridge Router 必须在普通 update owner 之前处理合法 `namespace_handoff` 描述符；该请求一旦路由到 handoff coordinator，普通 operation 不得保存 generation、下载后执行工件、写 Candidate/Activation 或取得 maintenance/reservation 所有权。
...
Manager CLI 先以闭合的静态命令表判定入口和参数形状，再为已知且需要运行态的命令执行身份 Router；未知命令、未知参数、重复 locator、相对或非规范化路径只返回用法错误，不能读取 journal、配置、secret 或其它部署状态。Bridge 的现役 source `manager.toml` 可以位于 installer 已记录的规范绝对路径；无 handoff journal 时，静态解析得到的唯一 `--config` 只能绑定完整 source 布局，并且仍须证明运行 inode 等于 source stable inode，它不是 profile 选择器。没有显式 `--config` 时，Bridge Router 从操作系统账户主目录固定解析 source profile 的 canonical config；Cleanup/target-only Router 则从同一账户固定解析编译期 target profile 的 canonical config，二者都不读取 ambient `XDG_CONFIG_HOME` 来改变 locator。非 helper 启动只以 profile-neutral 的最小解析读取同一快照中的 `state_home` 来定位中立 handoff 根；Bridge journal 存在时由唯一 terminal binding 选择 profile并与 argv config 逐字节比较，没有 journal 时只能使用该 binary 编译期 stage 对应的 baseline profile。两次配置观测必须绑定同一普通文件 inode 和内容快照，Router 选定的快照、解析结果与全部 `RuntimePaths` 必须贯穿 singleton、startup ownership 与 application 构造，不能按路径重开或从 `HOME`、basename、目标目录重算身份。helper 绑定只来自一次性 startup capability，Bridge terminal target 绑定只来自已验证 journal；Cleanup 普通 target 绑定只来自编译期 target-only stage。watchdog 始终显式携带同一 config 绑定，不存在无 `--config` 的历史例外。`update` 也不能选择交接路径：CLI 只向已路由 control socket 发起认证 `check`，由服务端按当前 stage 处理普通更新或 Bridge handoff。宿主 `preflight` 必须在同一 terminal global observation 下覆盖全部检查/创建；准入失败或 nonterminal journal 必须保持资源零写入。
...
回执门之前的 draft 不等于不可变存储。供应链必须用无环封印绑定除 `promotion.json` 自身外的整个 release 资产集，并用上述成功 publish attempt 的独立 Actions 出处证明钉住 `promotion.json` 自身在内的 GitHub asset identity。同名 Git tag 必须在 visibility 前已被证明精确指向 candidate commit，并在 visibility 变更前后紧邻时刻复验 GitHub release/asset identity 和 manifest schema 对应的完整镜像闭集。任一并发修改、额外或重名资产、错误 tag、删除镜像或可见性变化都必须在公开前失败关闭。visibility 已变更后才失败的镜像后验必须作为已公开发布事故报警，不能假装 workflow 失败等于可见性已回滚。

当前公开直接前任已经具备完整 `promotion.json` 与 Actions provenance；publisher 与 evaluator 都必须重新下载并逐字节复验该封印链。不存在无 provenance 的 current、历史摘要夹具、特殊资产目录或按 generation 放宽的兼容入口。
...
Manager 按 manifest schema 执行精确闭世界镜像校验：schema 1 的 source-owner/Bridge 固定为十个运行镜像加一次性交接 helper，schema 2 的 Cleanup/target-only 固定为十个运行镜像并禁止 helper 与 `namespace_handoff`。缺失、重复逻辑身份、额外镜像或 schema/profile 错配都会使整份 manifest 无效；不存在按历史 generation、摘要或本地路径启用的专用解码器。已经退出当前 Compose 的服务不能因 journal、manifest 键或 Docker 残留重新进入运行边界。

Manager 的预约从首次 Platform reserve 到持久 `maintenance=true`、再用同一 operation id 确认 reserve 后才可执行破坏性操作。响应不确定时只有明确 release 才能回到非维护状态；Manager 不可达或预约身份不一致时所有管理写操作 fail closed。Platform 启动任何有副作用 worker 前必须恢复同一持久预约状态。Handoff evidence 在持有业务准入锁时必须先以生成契约中的目录、记录和字节预算流式门禁，再排序或读取内容；canonical source inventory 的 count/bytes 同时是硬预算，不能先完成无界遍历或哈希再比较。

快照完整验证、候选 generation 核心 readiness、Manager watchdog 提交和 reservation release 完成前不能开放业务。reservation 的 `commit-release` 与 `abort-release` 是两个独立认证能力，普通与 handoff 请求都使用限大、拒绝重复/未知字段/尾随值的闭世界 JSON：前者只在普通更新 watchdog 已耐久提交，或命名空间 helper 已同步 forward-only `target_commit_planned` 后调用；后者只恢复准入，失败、取消、restart、repair 和 rollback 都不得获得 schema commit 能力。Gate 已成功但 finalized operation 与 pending state 尚未原子收敛时，恢复必须在清 state 前重放同一种幂等 Gate 结算，不得重复 SelfUpdate。Manager 只可从同一锁内的 state/finalize operation 快照投影显式 `gate_settlement`；字段缺失、损坏或错位都失败关闭，不能凭 Current、maintenance 或 journal 片段猜测。普通 operation 的首次 Gate action 必须与 `Finalized=true` 同次写入；install/update 只有 watchdog 已确认 candidate Manager 才能记录 commit，无 SelfUpdate 模式记录实际 abort，非 generation operation 不能记录 commit。候选验证保持纯读；workspace、marker、Runtime alias 与 Camoufox sidecar 必须从启动起已经符合 current schema，未物化、旧格式、缺失或身份漂移都失败，普通更新没有修复能力。核心 readiness 只包含 Manager 控制面、Platform 与 Agent Runtime；能力服务失败保持 degraded。Manager 自更新 activation/watchdog 仍是唯一持久安全所有者；Docker 清理只处理精确 ownership/project/resource label 且无消费者的对象，禁止全局 prune。Manager `/v1/status` 不投影 manifest、快照或其它宿主绝对路径。
...
终态 recovery 只有在 manifest source commit 与当前架构 Manager 完整 SHA-256 唯一确定 journal，且 transaction、受管路径、manifest/operation 原始摘要、原 Candidate、superseded 普通 plan 和 committed recovery plan 全部双向闭合时，才能只读充当 Platform finalize 证明。该证明读取 manifest、operation 与状态路径时必须始终使用 Router 已选定的 active technical profile，不能在 Bridge target 或 recovery 接力中退回编译期默认 profile。后继 recovery 还必须证明 Current、stable、运行 inode、metadata 一致且 Previous 精确等于该 journal 提交的 recovery Current；校验不得改写历史证据。只有同一未结算 finalize 满足这些条件时才可接力替换健康 Current，其余健康 Current 在停止服务前一律拒绝。
...
Multipart 正文必须增量读取并先写入 Platform 数据根内 owner-only 的请求 staging 目录；解析过程只保留边界探测所需的小型缓冲区，不得把完整请求或附件复制到内存。服务端完成数量、大小、配额、文件类型和摘要校验后再把 staging 文件流式提交到附件目录；请求成功、失败、取消或超时后都必须清理 staging。只有允许的位图格式可以内联给模型；其余附件通过当前 scope 的只读 Sandbox 挂载访问，target-only 路径为 `/workspace/.agent-platform/attachments`，Bridge source 路径只在交接前为 `/workspace/.ubitech/attachments`。Platform 不得把自己的数据路径写进普通 Run metadata；唯一例外是由可信配置派生、只进入当前 scope 系统提示的宿主工作区映射。Manager 不得把其它 scope 或全局附件根挂入 Sandbox。Agent 生成附件只能从当前 workspace、平台管理的媒体目录和显式媒体根返回，并在解析真实路径后再次校验。
