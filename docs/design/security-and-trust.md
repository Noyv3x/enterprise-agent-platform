...
当前 release 的闭世界镜像目录只有 canonical 容器契约中的十个现役镜像，不包含任何迁移 helper；Cleanup 后的 Manager、Platform、Runtime、用户和模型均没有调用、恢复或按历史摘要重新创建已退役迁移数据面的入口。
...
### Bridge→Cleanup 历史发布证据（非当前运行职责）

完整 source-owner/Bridge 基线当时必须在任何配置默认值、Manager state、普通 operation、自更新恢复、control 或 Gateway 初始化之前构造身份 Router。Bridge 的普通 source 启动只注入编译期 source profile；Bridge target profile 只能由外置 handoff journal 在 owner、类型、权限、schema、事务、阶段、源/目标 binding、运行二进制与 stable inode 全部验证后注入。manifest、CLI、环境变量、品牌、目标目录、二进制 basename 或 Docker 对象不得成为 profile 选择信号。Bridge Router 必须在普通 update owner 之前处理合法 `namespace_handoff` 描述符；该请求一旦路由到 handoff coordinator，普通 operation 不得保存 generation、下载后执行工件、写 Candidate/Activation 或取得 maintenance/reservation 所有权。
...
Bridge Manager CLI 当时先以闭合静态命令表判定入口和参数形状，再为已知且需要运行态的命令执行身份 Router；Bridge source `manager.toml` 可以位于 installer 已记录的规范绝对路径，但唯一 `--config` 仍须绑定完整 source 布局并证明运行 inode 等于 source stable inode。没有显式 `--config` 时，Bridge Router 从操作系统账户主目录固定解析 source canonical config，不读取 ambient XDG locator。非 helper 启动以同一 owner-controlled 配置 inode/字节快照定位外置 handoff 根；journal 存在时只有唯一 terminal binding 可选择 profile 并与 argv config 逐字节比较。helper 绑定只来自一次性 startup capability，Bridge terminal target 绑定只来自已验证 journal。Bridge `update` 通过已路由 control socket 的认证 `check` 进入普通更新或 handoff，宿主 preflight 则在同一 terminal global observation lease 下覆盖全部检查与创建；准入失败或 nonterminal journal 保持资源零写入。
...
以上 Router、source/target 参与者、coordinator、helper、handoff journal、`namespace_handoff`、`target_ack` 与专用 promotion receipt 仅说明已经完成的 Bridge→Cleanup 受控发布如何建立审计链。Cleanup 后的二进制不包含这些执行入口，不读取 source profile，也不能从终态 journal、旧目录或历史镜像恢复它们。精确历史状态机仍由[自动更新](../operations/auto-update.md#技术命名空间迁移发布)与[部署](../operations/deployment.md#技术命名空间交接)保存，不能据此扩大当前 Manager 权限。

### 当前 target-only 更新与恢复
