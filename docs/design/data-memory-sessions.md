...
`agent_scope_sessions` 是已退役运行路径的回滚残留，不属于当前数据库结构，也不承载 Pi Runtime 的上下文。版本化 scope 迁移发现该表时必须在替换 `agent_scopes` 前于同一事务中清理；当前会话及历史别名只以 `agent_runtime_scopes` 和 `agent_runtime_scope_sessions` 为准。

## Agent scope

规范私人 scope 为 `private:<user-id>`，频道主 Agent scope 为 `channel:<channel-id>:main-agent`。scope 保存稳定的相对 workspace 标识和不可由模型覆盖的主 Agent sandbox identity；Runtime lifecycle 和 session 可以独立轮换。委派 scope 继承父 sandbox identity，不建立新的工作目录。

每个 workspace 写入 `.ubitech-agent-scope.json`，记录 scope、lifecycle、sandbox identity 和逻辑执行边界。数据库不得保存容器内或宿主绝对 workspace 路径；Platform 在自己的数据根解析相对标识，Manager 将同一目录映射为 Sandbox `/workspace`。每次使用都重新检查路径组成与符号链接，缓存不得绕过。
