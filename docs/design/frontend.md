...
动效只解释位置和状态，无装饰入场、弹跳或主题渐变。电脑一次预留右侧宽度，仅新面板位移淡入；不动画列宽、flex-basis、输入或视口高度，不延迟卸载或租约释放。textarea 直接写入测得的有界高度。真实视口 resize 后，跟随者留底，阅读者保锚点且不增未读；stream、prepend、scope 切换即时收敛，不造消息或额外滚动动画。工作过程展开/收起以 grid 行 0fr↔1fr 与透明度过渡（约 300ms，`cubic-bezier(.23,1,.32,1)`），箭头同步旋转；收起期间内容保留但 inert，过渡结束才卸载；运行中仅头部文字微光与标记呼吸，不做多行持续动效。reduced-motion 实时作用组件、CSS 和文件呈现，不重挂 Store、丢输入或关面板（工作过程据收起时的计算样式立即卸载，微光改为静态文字）；组件动效 Provider 常驻，不通过切换组件树实现偏好，也不另加动画系统。
...
**工作过程**仅由真实 `tool`、`tool.started`、`tool.completed` 建立；`learning.review.*` 不生成聊天、输入中状态、过程或通知。工具开始后过程固定在回复顶部，头部是贴合内容宽度的单一展开按钮：标记、13px 中等字重标题、紧邻的 12px 元信息与箭头，无蓝色竖线或远端状态徽标。运行中默认展开，标题为简短状态“正在处理 / Working / 正在處理”，等待审批为“等待审批 / Awaiting approval / 等待核准”；含目标的真实状态文案作为头部描述与 tooltip。用户可收起为一行“当前任务 + 等宽摘要 · 步数 · 已用时”，该选择只在本 Run 内保持，后续更新（含最终输出开始流式显示）不重新展开或收起。完成/失败时仅收起一次：原位结算或切换为持久化记录时均从收起开始，不闪现展开的历史。焦点在过程内时，原位收起移回头部按钮；实时记录被持久化/错误记录替换时，焦点交给同一 run 的新头部（短暂等待稍后到达的持久消息，否则回到所在对话日志），用户已放在别处（如草稿）的焦点不被夺取。历史记录的展开状态按 run 记忆，标题为真实操作分类计数；失败为可见的“AI 工作过程失败”加分类计数。无工具的等待状态沿用同一头部样式（不可交互），保留排队/回复/审批的真实状态文案。运行中的过程头部和无工具等待状态显示自 `started_at` 起的已用时（`MM:SS`），只对当前 Run 计时，历史记录不计时；等待审批时头部以锁标替代微光标记。
