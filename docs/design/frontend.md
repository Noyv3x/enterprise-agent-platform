...
[Beautiful UI](https://www.beautifului.dev/)（MIT，源码与改编说明在 `components/ui/beautiful/`，许可随 `public/licenses/` 分发）是 AI 交互界面的成品基线：工作过程（Thinking + Tool Chips）、等待态（Loading State 像素网格 + 微光标签）、Composer（Prompt Bar）、代码块（Code Block）、审批卡（Approval Card）、侧栏（Sidebar Nav）、分区标签、资源列表与状态标签（Task Rows）、消息操作与流式 caret（Streaming Text）均采用其源码的生产化改编，只接收真实状态、本地化文案与回调，不带演示数据、定时脚本、远程图片或付费图标。Beautiful UI 没有成品的表单、选择器、确认、Drawer/Dialog 与消息提示仍由 Ant Design 承担，外观经 Token 对齐同一组设计值（药丸按钮、灰底输入、10px 菜单、环形阴影）；不复制 Ant 内部 DOM 或叠加高优先级补丁。按钮文案按原样显示，不插入 CJK 字间空格。共享组合仅接收本地化内容、真实状态、回调、插槽，不访问 API、复制 Store 或自造数据；领域 CSS 共用 Token。Composer 的原生 textarea、mention option、隐藏文件 input 保留 IME、caret、焦点及会话附件状态机。共享状态语言只有一套：细环（`Spinner`/`StatusMark busy`/工作步骤）表示实时工作，勾/警告表示完成/失败，`LoadingState` 用像素网格加真实标签；分类标记（如公共）用图标代替状态点；资源列表（记忆、技能、定时任务、审计、账户等）为同一卡片内的分隔行，可带前导族标，状态胶囊置于标题行右侧；流式正文末尾只在已收到字符后显示一个细 caret，不虚构文字。

### 排版、密度与颜色

| 维度 | 基准 |
| --- | --- |
| 字体/密度 | 自托管 Inter 与 JetBrains Mono 的拉丁子集（随静态资源分发，不请求第三方字体），中文、中文标点与其余字符落到系统 CJK 字体；等宽仅用于代码、命令、时间和标识。正文/表单 14px，辅助文字 12–13px，对话约 15px、行高 1.6，页/区标题约 18/15px，字重只用 400/500/600；英文界面字距约 -0.01em，中文与长文保持原字距。桌面导航约 232px，对话约 760px 且与 Composer 同轴。桌面/平板/手机留白约 24/20/16px，区距约 12–24px，桌面控件 27–32px（Beautiful UI 尺度），触屏/窄屏目标至少 44px；圆角 6px 标签、8px 控件、10px 卡片与菜单、14px 窗口、药丸按钮。 |
| 颜色/信息 | 采用 Beautiful UI 语义色（page、canvas、surface、inset、field、hover、ink 三级、line 与语义色），由调色板统一提供给 Ant、设计系统与组件，无暖纸或绿色偏色。为满足 4.5:1，弱文字与状态色比上游略深；分隔靠色阶与平滑环形阴影而非描边，输入框为灰底填充。导航与分区标签的选中态为中性灰底，品牌色只用于主操作、链接、发送与焦点；状态标签为同色浅底胶囊，状态不只靠颜色。品牌操作填充保持原值，强调文字在内容、选中、hover 底色上均达到 4.5:1，派生色与混合底色共用计算；强制颜色模式为无描边表面补实线边。保存留在所属表单，权限、风险、引导、错误和危险确认不能删减。 |

### 动效与阅读连续性

动效只解释位置和状态，无装饰入场、弹跳或主题渐变（切换主题的帧内停用过渡，颜色直接切换）。电脑一次预留右侧宽度，仅新面板位移淡入；不动画列宽、flex-basis、输入或视口高度，不延迟卸载或租约释放。textarea 直接写入测得的有界高度。真实视口 resize 或内容在提交后增高（过程展开、延迟排版、画中画留白）时，跟随者留底，阅读者保锚点且不增未读；stream、prepend、scope 切换即时收敛，不造消息或额外滚动动画。工作过程展开/收起以 grid 行 0fr↔1fr 与透明度过渡（约 300ms，`cubic-bezier(.23,1,.32,1)`），箭头同步旋转；收起期间内容保留但 inert，过渡结束才卸载；运行中仅头部文字微光与步骤细环旋转，不做多行持续动效。reduced-motion 实时作用组件、CSS 和文件呈现，不重挂 Store、丢输入或关面板（工作过程据收起时的计算样式立即卸载，微光改为静态文字）；组件动效 Provider 常驻，不通过切换组件树实现偏好，也不另加动画系统。
