...
| 维度 | 基准 |
| --- | --- |
| 字体/密度 | 本地无衬线，不下载字体（已安装 Inter/JetBrains Mono 时优先，否则落到系统与 CJK 字体）；等宽仅用于代码、命令、时间和标识。正文/表单 14px，辅助文字 12–13px，对话约 15px、行高 1.6，页/区标题约 18/15px，字重只用 400/500/600；英文界面字距约 -0.01em，中文与长文保持原字距。桌面导航约 232px，对话约 760px 且与 Composer 同轴。桌面/平板/手机留白约 24/20/16px，区距约 12–24px，桌面控件约 32px，触屏/窄屏目标至少 44px；圆角 6/8px 控件、14px 卡片、16px Composer 与用户气泡。 |
| 颜色/信息 | 视觉语言参照 Beautiful UI：中性冷灰、低彩度，无暖纸或绿色偏色。浅色为近白画布、白色阅读区与卡片；深色为分层中性灰（画布、阅读区、卡片/浮层逐级提亮）。文字三级（主、次、弱）均在所在底色上达到 4.5:1；分隔靠色阶与柔和环形阴影而非描边，输入框为灰底填充，卡片、Composer、浮层用环形阴影。导航与分区索引的选中态为中性灰底，品牌色只用于主操作、链接和焦点；状态标签为同色浅底，状态不只靠颜色。品牌操作填充保持原值，强调文字在内容、选中、hover 底色上均达到 4.5:1，派生色与混合底色共用计算；强制颜色模式为无描边表面补实线边。保存留在所属表单，权限、风险、引导、错误和危险确认不能删减。 |
