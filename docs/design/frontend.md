...
- 浏览器“接管”必须由用户手势启动，显示服务端返回的租约状态，并把租约绑定到取得时的 `tab_id` 与本地到期时间，不能把旧租约重新解释为轮询后出现的当前 tab。人工输入、取得和释放使用同一客户端串行队列，单调 sequence 只能按队列顺序发送；不得用并发 fire-and-forget 请求让点击、按键、滚动或释放乱序。结束、窗口失焦、页面隐藏、租约到期、tab 变化、组件卸载或服务端返回租约冲突时立即恢复只读，并对原 tab/lease 做尽力释放。终端预览永远不可发送输入。
