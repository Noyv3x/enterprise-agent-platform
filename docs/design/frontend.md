...
Ant Design 统一基础控件、状态、确认及 overlay，Token/CSS 单一所有权；不复制内部 DOM、自造基础组件或叠加高优先级补丁。共享组合仅接收本地化内容、真实状态、回调、插槽，不访问 API、复制 Store 或自造数据；领域 CSS 共用 Token。Composer 的原生 textarea、mention option、隐藏文件 input 保留 IME、caret、焦点及会话附件状态机。共享状态语言只有一套：进行环（`Spinner`/`StatusMark busy`）表示实时工作，勾/警告表示完成/失败，`LoadingState` 用同一进行环加真实标签；资源列表（记忆、技能、定时任务、审计、账户等）为独立卡片行，可带前导族标，状态置于标题行右侧；流式正文末尾只在已收到字符后显示一个 caret，不虚构文字。
...
Markdown 支持 GFM、`$...$` 行内公式、独占行 `$$` 块公式及 `\$` 字面美元。KaTeX 排版与 MathML 随正文懒载，禁用可信命令；无效或流式未闭合公式保留源码/有界错误，不使消息树崩溃。原始 HTML、远端图片和不安全 URL 的边界不变。单源码换行安全显示，普通块折叠 AST 空白，代码保留原空白；各块采用紧凑一致间距，首尾不撑高，公式只在内部横滚。Agent 无气泡、用户气泡按内容收窄并靠右（桌面至多 85%），与 Composer 同轴；私人 Agent 不显示作者名，频道保留展示名。复制、撤回及 Composer 的添加文件/发送为图标按钮，可访问名称与 tooltip 保留完整文案，复制结果经 polite 播报；发送/生成中等实时状态以进行环标记。用户副标题仅用非空职位或原有 `@username`，不以权限组或角色兜底；管理权限列独立。

**工作过程**仅由真实 `tool`、`tool.started`、`tool.completed` 建立；`learning.review.*` 不生成聊天、输入中状态、过程或通知。工具开始后过程固定在回复顶部，运行中（含最终输出开始流式显示后）保持紧凑可见，不提供折叠或详情控件；完成/失败后自动收起，键盘入口为“查看 AI 工作过程”。运行中的过程头部和无工具的等待状态显示自 `started_at` 起的已用时（`MM:SS`），只对当前 Run 计时，历史记录不计时；等待审批时头部以锁标替代进行环。
...
- 每行仅显示工具、单行摘要和状态，合并重复、匿名、内部噪声及审批。行首以状态标记区分运行中（进行环）、已完成（勾）与失败（警告），文字状态同时保留，不只靠颜色或动效；工具名前带文件/终端/搜索/浏览器族标，路径、命令与查询摘要用等宽。展开时间线后，仅有真实脱敏参数、结果、错误或阶段正文的项可展开。文件优先操作、唯一路径及正文/结果；terminal/process 优先完整脱敏命令和输出；搜索优先查询与命中；MCP 显示 server/tool，不显示环境值或已省略的外部结果；其它工具优先安全动作和结果。状态不重复，时间置末。
...
| 状态 | 契约 |
| --- | --- |
| 默认 | 阅读区右下、实际 Composer 上方放置 **16:9 浮动画中画**，与回底按钮同处透明绝对定位层，**零文档流高度**，无整行白占位；空白部分穿透 pointer 和滚轮。回底按钮水平居中于阅读列，画中画靠右，二者不重叠。未读数有界，按钮有清晰 tooltip/aria；短窗、长输入和缩放也不挤掉阅读区。 |
...
| 手动隐藏 | 画中画右上有“隐藏 AI 的电脑”按钮（三语见上表）；隐藏后同一 Run 及其保留资源不再显示画中画，页头“AI 的电脑”入口仍可展开。仅当本 scope 出现**不同 run_id 的新工作**时自动重新出现；换 scope 清除隐藏记忆。隐藏后焦点回 Composer。 |
