...
普通文件附件显示文件名、类型、大小和下载动作。XLSX 附件额外显示同源鉴权的只读表格预览：卡片右上角固定提供展开与下载两个图标按钮，内嵌区域只展示有界的首屏行列并允许自身横向滚动；展开后在组件库弹窗中查看服务端返回的有界工作表内容。预览加载、空表、截断和失败都有明确状态，失败不能影响原文件下载。前端只把服务端返回的单元格字符串作为文本渲染，不解析公式、不执行宏、不在浏览器引入第二套 Office 解析器。
