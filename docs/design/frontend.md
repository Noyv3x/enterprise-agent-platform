...
Ant Design 统一基础控件、状态、确认及 overlay，Token/CSS 单一所有权；不复制内部 DOM、自造基础组件或叠加高优先级补丁。共享组合仅接收本地化内容、真实状态、回调、插槽，不访问 API、复制 Store 或自造数据；领域 CSS 共用 Token。Composer 的原生 textarea、mention option、隐藏文件 input 保留 IME、caret、焦点及会话附件状态机。
