...
聊天 Composer 的自动增长 textarea、mention option 和隐藏文件 input 属于输入状态机的一部分，需要维持中文 IME、caret、焦点和分会话附件恢复，因此可以保留原生节点，但必须使用统一 Token、可访问性语义和组件级回归测试。知识 Embeddings 配置表单使用组件库的标准表单、密码输入、数字输入、状态和错误语义，不保留任意 `.env` 字段编辑器。除 Composer 的明确边界外，文件选择使用组件库 Upload 语义，不能再新增原生交互控件。
