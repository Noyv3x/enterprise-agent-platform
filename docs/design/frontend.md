...
设置页提供浏览器通知开关。权限只能由用户手势请求；仅在页面已加载完成、用户显式启用、权限为 `granted` 且页面隐藏或失焦时，对任一当前用户可访问 scope 中新持久化完成的 Agent 回复弹一次通知。通知由独立于当前视图和消息数组的轻量完成事件流驱动：事件必须携带单调持久消息 ID、scope 类型与 scope ID；首次连接只建立当前水位，网络重连使用 SSE `Last-Event-ID` 延续该水位。初始 hydrate、历史分页、重复 SSE、切换后首次加载 scope 和当前可见页面不通知；点击通知聚焦页面并定位对应 scope。页面保持打开时，隐藏状态继续保持完成事件流；切换频道或切到私人对话、知识库、设置和管理页都不能丢失其它 scope 的完成事件。该能力不是浏览器完全关闭后的 Web Push；页面重新加载后重新建立当前水位，不补发页面关闭期间的回复，非 secure context 时明确显示不可用。
