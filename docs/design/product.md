...
私人 Agent 和每个频道主 Agent 都是长期逻辑身份，拥有稳定工作区和可轮换的会话生命周期。逻辑身份可以先于首次执行登记；只有受认证更新提交可把旧基线遗留的未物化工作区补齐，普通启动不会静默修复缺失目录。私人消息按进入顺序持久化；同一用户连续发送的相关短消息可并入正在运行的回复，并保留每条消息的处理状态。频道消息、私人消息和 Agent 回复都保存在平台消息库中；Agent 在工作区生成的文件由 Platform 将规范逻辑交付标记转换为当前消息的结构化附件，界面不直接信任或解析正文路径。用户可以撤回自己已经发送并持久化的频道消息；撤回只改变频道中的可见性，不取消已经排队或运行的 Agent 回复，也不删除 Runtime 历史、附件或审计数据。

认证用户进入产品后的默认会话是自己的私人 Agent；只有账号没有私人 Agent 权限时才回退到第一个可读频道。频道仍是显式导航目标，不能因为频道存在就覆盖私人 Agent 默认入口。

界面只在发生工具调用时展示工作记录。工作过程先展开显示在当前回复顶部，最终输出开始后自动折叠；没有工具调用时不创建空工作记录。审批请求是工作过程的一部分，但必须保持为明确、可操作的独立状态。

## 核心能力

- 频道、私人对话、附件、输入合并、流式回复、上下文用量查看，以及不进入模型或产品消息的本地 `/compact` 会话压缩命令。
