...
私人 Agent 和每个频道主 Agent 都是长期逻辑身份，拥有稳定工作区和可轮换的会话生命周期。逻辑身份可以先于首次执行登记；只有受认证更新提交可把旧基线遗留的未物化工作区补齐，普通启动不会静默修复缺失目录。私人消息按进入顺序持久化；同一用户连续发送的相关短消息可并入正在运行的回复，并保留每条消息的处理状态。频道消息、私人消息和 Agent 回复都保存在平台消息库中。
