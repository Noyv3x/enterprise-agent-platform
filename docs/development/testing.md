...
完整本地门禁在普通 Git 工作树中还会以 `HEAD → WORKTREE` 运行文档双向同步检查，因此未提交的代码、文档、暂存和未跟踪文件都进入验收。不得用 `HEAD → HEAD` 的空变更检查代替这一边界。

该命令先校验文档与生成契约，再运行发布脚本、Manager、Python、Agent Runtime 和前端测试与构建。前端构建会同步受版本控制的静态资源；提交前必须再次确认这些生成变化已纳入变更。仓库不提供第二个顶层测试入口。
...
镜像身份与 Manager 二进制上传允许同一 workflow run 的全量重跑覆盖其同名中间 artifact。真正组装 GitHub Release 的 `publish` job 必须按 `prepare` 已解析的完整 source commit 跨 run 串行，不能使用可能指向同一提交的原始分支/ref 文本作为锁键；它只能生成并封印 draft，已公开 release 仍逐文件比较并拒绝漂移。封印测试必须覆盖除 `promotion.json` 外的精确资产名称/SHA-256/size，并对任一内容、大小、未知或重名资产漂移失败关闭；GitHub API 身份在上传后、promotion 紧邻前和切换后必须一致。每个成功 publish attempt 还要上传闭世 Actions 出处证明；测试覆盖 run id/attempt、workflow path/event/repository、Container execution head、built source commit、Quality run/attempt/workflow/head、dispatch ref/source 规则及 release/asset API identity 的任一错配，明确覆盖 default branch 已前进但合法旧 candidate、无关 run 唤醒、proof source 与上游 Quality head 不符，以及封印后、证明前中断的新 attempt 只能复验后补上自己的证明。promotion 门还要用无凭据 registry 请求覆盖全部十一个受管镜像、摘要不符、私有/缺失和超时，并对 visibility 前后两次复验的缺失任一失败。同名 tag 测试覆盖不存在时 create-only 建立、精确 commit 复用、错误 commit、annotated tag 与竞争，并保证任一错配都发生在 visibility 修改前。main channel 的唯一 promotion evaluator 使用独立全局锁，测试必须证明它只选择唯一直接后继、不能跳过或重排 source-owner→source-owner 稳定化后继→bridge→cleanup→target-baseline→target-baseline；同阶段后继只能在 Bridge 前直接跟随 source-owner，并保持 transition/source/target profile，`source_owner_compat` 仍只绑定唯一 P1 而不能随前任泛化。source-owner 与 target-baseline 属于普通自动 promotion，只有 bridge 与 cleanup 必须经过签名部署回执门。回执测试还须覆盖闭世界 schema、Ed25519 公钥类型、Manager 摘要、过期时间及永久 challenge 防重放 ref。GitHub API 分页结果必须先聚合各页再做闭世界计数，不得把串联的多个 JSON 页误当单页。

P1 workspace 目录发布测试必须覆盖首次 rename 与 exact-final 崩溃重放的同一耐久屏障，并断言顺序为 child fd、staging/source parent、destination parent。stage 已不存在或 exact 空 residue 已清理时也不得省略任何一次 `fsync`；在 staging parent 或 destination parent 注入一次失败后，本次必须返回 committed error，下一次重试仍须重新触达完整屏障后才能提交 marker/alias 与释放 reservation。final 缺失、错误 inode、未知或非空 residue、元数据漂移和 rename 后目录被写入均应保留证据并失败关闭。

A2 首次 promotion 还必须以真实 P1 release API fixture 和真实 P1 manifest/Compose fixture 覆盖唯一窄兼容：精确 generation、公开非 draft、八个旧资产及其 ID/digest/size、contract 固定的 manifest/Compose 摘要与十镜像闭世界必须通过；增加 `promotion.json`、替换任一固定字节、错误 generation、未知资产或把同一逻辑应用到其它 current 都必须失败。测试不能为了让 P1 通过而伪造它从未拥有的 provenance。

release-transition 测试还必须证明长期 control API 没有 identity、私钥或签名路由，只能在认证后返回闭世界的无机密 observation；宿主 CLI 才能创建和使用密钥。状态根、challenge 与输出测试覆盖任一父组件符号链接、既有宽权限目录、硬链接、路径/inode 并发替换、失败回滚和父目录同步；清理只能删除本次创建且仍为同一 inode 的对象，替换对象必须保留并失败关闭。测试还要把 `$HOME` 指向无关目录，证明默认状态与 Manager stable path仍来自 UID 匹配的操作系统账户记录。
