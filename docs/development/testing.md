...
- 电脑画面：空闲／仅排队且无资源时不出现空白监视器；实际回复或审批开始、尚无工具线索时默认显示等待中的画中画，不虚构文件。文件／终端／浏览器／搜索／呈现页随真实内容切换，紧凑、桌面右侧区域和窄屏侧滑只允许一个实时消费者。明确点击才展开，状态推送不自动放大；桌面聊天继续可操作，收起／scope 切换释放资源并恢复合理焦点，窄屏沿用模态焦点与 Escape 行为。数据到达后必须显示真实内容，打开画面默认只读；连续 draft revision 快于正文请求时保持单在途并最终追到最新版，只对权威字符渐进显示，非前缀替换与 reduced-motion 安全收敛。文件 started 的 404 保持等待，completed 后显示最终正文，patch 刷新旧快照；短终端命令使用真实有界最终输出；HTML 完成前不挂载，完成后同路径重写可刷新，iframe 不含 `allow-same-origin`。三语电脑名称、独立记忆／技能／任务、显式接管、发送前释放及旧 scope／Run／工具请求拒绝均保留。16:9 小卡片、右半屏几何、输入不遮挡、布局切换后的阅读位置及普通／减少动态效果在真实浏览器验证，不以 CSS、源码或固定像素断言替代；
