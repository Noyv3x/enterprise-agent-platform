...
手动驱动计划任务领取的测试必须只有一个领取者：在构造测试服务前隔离自动调度线程的启动，仍调用真实 dispatcher，并保留实际 Agent 作业执行、终态同步与暂停／幂等断言。不得让后台计时器抢先领取后，再把正确的零领取结果误判为失败；也不得通过增大等待时间或模拟被测 dispatcher 返回值掩盖竞争。
...
SQLite 测试使用临时数据目录，不共享开发数据库；事务测试必须覆盖正文异常与 `commit` 异常都执行 rollback，并证明同一线程连接随后仍可安全写入。Platform 启动安全回归还必须对 profile 实例锁、SQLite 主文件和已有 WAL/SHM 分别覆盖 symlink、hardlink、owner/type/link-count 异常与取锁/建立连接窗口的确定性 inode swap；断言异常发生在 PID、基线、schema 或 WAL 写入前，替换对象和外部链接 inode 保持原样。OAuth、Telegram、Firecrawl、SearXNG、Camoufox 和 Git 操作优先使用确定性 fake；没有显式凭据和服务时，不把真实网络集成作为单元测试前提。MCP 测试使用本地合成 stdio server，覆盖配置热读、不同 workspace 隔离、argv 启动、初始化、tools/list、tools/call、审批、协议/超时/输出上限和不可信结果，不访问真实服务。审批回归必须覆盖超过通用预览深度/数量阈值的普通参数仍完整展示、敏感字段脱敏、不可见/双向控制字符硬拒绝和完整展示超限拒绝；执行回归必须证明 Manager 保留快照、输出文件、终端预览与 Runtime 工作事件不出现可逆 MCP 请求或原始结果，而当前 Manager→Runtime 响应仍能交付结果。客户端路径回归还必须覆盖 config/cwd/工作区 command 的 fd 固定与实际启动，防止父目录替换让已校验的路径在使用时越出 workspace。
...
上下文计量回归必须覆盖：仅系统提示或工具 schema 使完整请求越过阈值时及时压缩；有效供应商用量不重复计入请求固定开销；恢复历史与压缩后保留 tail 中的旧 usage 不得导致低估或反复误触发压缩；终态缺失 usage 时只估算现役 handoff + tail 及固定开销，图片不按 base64 长度计费。摘要完整性回归必须分别以输出上限截断、正常结束但正文超限和不合法工具调用证明失败发生在归档/替换前，journal/archive 保持原字节且 session 门闩可再次使用；同一共享摘要路径在自动压缩失败时也不得发布 `context.compacted` 或把半截摘要持久化。
...
Vitest 使用最多两个 worker，避免重量级 jsdom／组件库初始化与全量门禁中的 Python、Runtime 等任务争抢全部 CPU，造成与业务无关的超时。非动效用例使用真实的 reduced-motion 偏好分支；文本只是测试前置条件时可以使用真实粘贴操作，按键、IME 和发送状态机用例仍保留逐键交互。不得增大用例超时、削弱断言或删减并发场景来获得通过。
...
- 电脑画面：空闲／仅排队且无资源时不出现空白监视器；实际回复或审批开始、尚无工具线索时默认显示等待中的画中画，不虚构文件。文件／终端／浏览器／搜索／呈现页随真实内容切换，紧凑与始终占据右半屏的非模态区域只允许一个实时消费者；窗口缩放不重挂载查看区域、不重放接管请求或无故释放现有租约。明确点击才展开，状态推送不自动放大；聊天继续可操作，收起／scope 切换释放资源并恢复合理焦点，Escape 仅在区域内未被子控件消费时收起。数据到达后必须显示真实内容，打开画面默认只读；连续 draft revision 快于正文请求时保持单在途并最终追到最新版，只对权威字符渐进显示，非前缀替换与 reduced-motion 安全收敛。文件 started 的 404 保持等待，completed 后显示最终正文，patch 刷新旧快照；短终端命令使用真实有界最终输出；HTML 完成前不挂载，完成后同路径重写可刷新，iframe 不含 `allow-same-origin`。三语电脑名称、独立记忆／技能／任务、显式接管、发送前释放及旧 scope／Run／工具请求拒绝均保留。真实浏览器必须覆盖长对话滚离底部、矮窗口和等效浏览器缩放：跳转按钮和画中画不降低阅读区高度，不出现大块白色占位，画面无上下元数据白条、HTML 缩略视口完整、输入不遮挡、展开始终位于右半屏；同时检查普通／减少动态效果及阅读位置，不以 CSS、源码或固定像素单元断言替代；
- 手机动态视口、长代码/表格和 Composer 不扩大页面；
- 动态偏好在应用打开后切换时，未发送输入、展开状态和焦点必须保留；布局引起的 scroll 事件先于 resize 通知时仍保持原来的跟随／阅读意图，不把尺寸变化算成新消息。jsdom 不能替代浏览器原生 `inert` 键盘顺序和实际过渡生命周期的验证；
- 三种 locale 的 key 完整性；
- 更新维护页在 Store/登录失败时仍可接管。

完整视觉重建须在真实浏览器覆盖登录、导航、对话、上下文能力、设置和所有管理资源，以及桌面／手机、浅色／深色和加载／空／错误／可用状态。纯 CSS、源码文本、旧类名、几何和组件实现断言应删除，不改成另一组新设计钉死断言；保留真实操作、权限、输入状态机、内容安全与异步身份隔离回归。只读或外部能力不可用的本地预览应明确说明边界，不伪造服务健康或 Agent 结果。
