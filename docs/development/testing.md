...
- 电脑画面：空闲对话不出现空白监视器；文件／终端／浏览器／搜索／呈现页随当前可见工具切换并共用当前 scope 的查看能力，紧凑与展开只允许一个实时消费者。数据到达后必须显示真实内容，打开画面默认只读；连续 draft revision 快于正文请求时保持单在途并最终追到最新版，只对权威字符渐进显示，非前缀替换与 reduced-motion 安全收敛。文件 started 的 404 保持等待，completed 后显示最终正文，patch 刷新旧快照；短终端命令使用真实有界最终输出；HTML 完成前不挂载，完成后同路径重写可刷新，iframe 不含 `allow-same-origin`。三语电脑名称、独立记忆／技能／任务、显式接管、发送前释放及旧 scope／Run／工具请求拒绝均保留。紧凑比例、轨道宽度和旧抽屉位置不再作为行为测试；
