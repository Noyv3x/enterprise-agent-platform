...
`docs/` 是 Agent Platform 当前设计的唯一真相源。设计约束、运行边界、公开契约和运维行为应先在这里修改，再落实到代码与测试。根目录 `AGENTS.md` 只提供与工具无关的工作行为和“先读、先改文档”的入口，不得复制代码库结构、配置值或产品设计；`claude.md` 可以作为工具兼容入口，但只能引用 `AGENTS.md`，不能形成第二份规则或设计来源。
...
- 生成模块与机器可读契约逐字节一致；
- `docs/` 中的本地相对链接没有失效；
- `AGENTS.md` 只包含行为准则并纳入文档治理域；工具专用兼容入口不复制这些规则或 canonical 设计。
