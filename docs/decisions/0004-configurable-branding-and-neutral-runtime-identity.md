...
内部持久身份也采用中性命名，但不能在普通应用更新中原地批量替换。迁移使用两次不可变发布：桥接发布在 Manager 空闲且没有 candidate、activation、watchdog、finalize 或活动 Sandbox 调用时，建立持久 journal 和恢复快照，停止写入者，把 Manager、systemd unit、配置与数据根、Docker 项目和网络、secret/socket、数据库及工作区标记交接给中性身份；验收 Platform、Runtime、数据库、工作区、集成服务、自动更新与回滚后才退役旧身份。随后基线发布删除桥接入口和旧名称兼容，只保留中性运行身份。
