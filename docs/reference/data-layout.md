...
Bridge 对 source `data/platform.db` 的 checkpoint 还有独立的 Linux leaf 绑定契约。在 SQLite 以任何可写模式打开路径以前，helper 必须逐段 no-follow 打开并固定 `data/` 父目录 fd，再用 `openat(O_RDWR | O_NOFOLLOW)` 固定已存在的 `platform.db` leaf；父目录必须是部署 UID/GID 拥有的 `0700` 真实目录，leaf 必须是同一 UID/GID 拥有、同 device、`0600`、单硬链接的真实普通文件。任一已存在的 `platform.db-wal`/`platform.db-shm` 在 SQLite 触碰前也必须通过相同的 leaf 安全条件。helper 在整个 checkpoint 中保持父目录和主文件 fd，SQLite 只能从该已固定父 fd 派生的 Linux proc-fd 路径打开已存在的主文件，不得使用会创建缺失数据库的 `rwc`。第一个 SQLite 写动作紧前及连接关闭后，都要把 parent path/parent fd、parent-fd 下的 leaf 目录项和 pinned leaf fd 的 device/inode、owner、type、mode、link count 重新交叉核对；任一 inode drift、符号链接、硬链接或身份变化都在继续写入前失败关闭。当前 SQLite driver 没有直接消费已打开 fd 的受支持 API，因此这个边界是“固定父目录 + 固定 leaf + 写前/写后复核”，不声称消除同 UID 攻击者在两次观测之间替换后又恢复路径的全部 TOCTOU；安全性还依赖 checkpoint 前的 writer fence 和 owner-only 父目录，未来若引入可消费 fd 的 SQLite VFS 才能收紧该剩余窗口。
...
|---|---|---|
| SQLite `platform.db` | 在停止 writer 且完成 WAL checkpoint 后从单一完整主文件建立 target 副本。桥接在同一 SQLite 事务内把唯一 `schema_migrations` 基线名称从 `ubitech-agent-container-baseline-v2` 改为 `agent-platform-container-baseline-v1`，并且只把 `settings.key` 中三个机器自有键精确映射：`ENTERPRISE_SESSION_SECRET` → `AGENT_PLATFORM_SESSION_SECRET`、`ENTERPRISE_TELEGRAM_BOT_TOKEN` → `AGENT_PLATFORM_TELEGRAM_BOT_TOKEN`、`ENTERPRISE_TELEGRAM_WEBHOOK_SECRET` → `AGENT_PLATFORM_TELEGRAM_WEBHOOK_SECRET`。源键不存在时不创建目标行；source 副本中任一目标键已存在（无论对应源键是否存在或值是否相同）都是混合/冲突身份，整个事务失败关闭。版本号必须等于签名桥接 manifest 的 `database_schema_version`。当前 schema 的 workspace、附件和 Runtime identity 本来就是相对标识，不允许为了“看起来完成迁移”搜索或改写其它 TEXT/BLOB | `integrity_check`、`foreign_key_check`、`journal_mode`、完整 `sqlite_master` 结构、账号/消息/记忆/会话/附件等每张权威表的行数与有主键表的主键集合。三个映射行的 `value`、`secret`、`updated_at` 必须逐字段不变；其它设置与所有用户内容逐字节不变，即使正文或自定义键包含 source 文本也不改写。健康检查只需取得第一个完整性错误或外键违例就必须失败，不得在 evidence 进程中物化全部违例。source 的 `-wal`/`-shm` 只作为 checkpoint 前一致性集合验证，不能复制到 target |
| Agent Runtime | 当前闭世界只接受 `sessions/`、`approvals/always.json` 与 `idempotency/index.json`；保留 scope、session、lifecycle、approval 与 idempotency id。每个 JSON/JSONL 由对应版本 reader 校验，当前格式没有 technical-profile 或绝对宿主路径，因此文件字节保持不变；`logs/` 是明确登记的临时工件且不迁移。当前 source-owner 数据仍可带 canonical `p1_source_handoff` 登记的 `app/`、空 `home/`、空 `memory/` 与单一 `migration/hermes-cutover.json`：它们只是已退役的 Runtime checkout/迁移证据，Bridge evidence 与 transformer 必须按完整库存、owner/mode/link、install/package identity、四个精确 npm 相对 symlink 和 hermes-cutover 闭世界 schema 纯读验明，再在 drain 后整体省略；普通 Runtime 或更新路径不读取这些目录 | JSON/JSONL 逐条可解析、目录哈希与记录 identity 对应、引用的 scope/session 存在、幂等结果与审批状态不变；transcript、模型输入输出、工具正文和错误文本不搜索替换；当前根、明确临时根、精确登记的 source 退役根以外的顶层节点或未知机器字段失败关闭。机器 schema/version 必须是 JSON integer，布尔值和浮点表示均不等价。JSONL 单文件最多 256 MiB、最多 1,048,576 条非空记录；实现忽略纯空白行并在解码前剥离行首尾空白。Runtime identity 总数最多 16,384，Platform evidence 的 SQLite identity 查询必须在 SQL 层以“限额 + 1”截断后才物化；任一目录最多枚举 65,536 个直接子项，枚举必须流式计数后才排序。退役 checkout 在生成契约中的精确库存条目和非目录字节计数同时也是遍历预算：超过预算必须在继续递归或读取文件内容前拒绝，Platform evidence 与 transformer 必须消费同一份生成契约和一致测试向量 |
...
| workspace / `agent-envs` / `agent-skills` | 用户工作文件原样复制；每个 SQLite 权威 workspace 根把平台内部目录 `.ubitech/` 闭世界映射为 `.agent-platform/`，其下 Agent 用户数据只改这一段目录前缀，文件 bytes、mode、mtime 与可执行位不变；workspace 根另把平台拥有的 `.ubitech-agent-scope.json` schema 1 变换为 `.agent-platform-scope.json` schema 1，并只改 `technical_profile`。`agent_scopes.lifecycle_id` 是稳定 logical scope 元数据，`agent_runtime_scopes.lifecycle_id` 是可独立轮换的当前 Runtime conversation lifecycle；workspace marker 明确绑定后者，不得用前者代替。Bridge 必须从同一次只读 SQLite 观察中取得 scope 元数据与当前 Runtime lifecycle，并证明当前 `(scope_key, runtime lifecycle, session)` 存在于 durable alias 表；source/target 内部目录或两个 marker 同时存在均为冲突。Bridge evidence 开始前所有登记 workspace 都必须已经物化并符合 current marker 与 Runtime alias 规则；缺失或旧格式直接失败，交接不创建或修复 source workspace。`agent-envs` 与 `agent-skills` 全树按普通文件复制，不解释用户内容；`logs/` 子树不进入 target | workspace id、映射后的相对路径、除 source marker 外的文件内容和元数据按契约保持；marker identity 必须与 SQLite logical scope 行及其当前 Runtime lifecycle 一致。logical scope lifecycle 与 Runtime lifecycle 即使不同也必须分别逐字节保留；不得把测试 fixture 中相同的占位值当作二者相等的不变量。唯一 owner 例外是 canonical source inventory 登记的 root-owned 空 `attachments/` 挂载占位：它只可位于 source 内部目录的精确直接子路径、必须为 `0755` 空目录；若其 `.ubitech/` 父目录也是 root-owned，则整个父树只能由这一个空占位构成。两者在 target 归一为部署身份；其它 root-owned 路径失败关闭，不能把 root 加入整个 workspace 的泛化 owner 白名单。用户创建的其它同名文本、嵌套目录或源码不按名称猜测修改，target 冲突直接拒绝 |
...
canonical `p1_source_handoff` 记录的是当前唯一 source-owner 的即时目录库存，不是普通更新兼容入口；不能把“未作为资源复制”误写成“任意文件均可忽略”。Bridge 每次构造或重放请求都必须复验根、`data/`、`data/runtimes/`、四个固定 Runtime 根以及 `manager/` 的完整直接子项集合；未知、缺失或类型变化均在创建 staging 前失败。其处置语义固定如下：
...
| `data/.home/`、SQLite WAL/SHM 与实例锁、`upload-staging/`、各固定 Runtime 的 `cache/`/`logs/` 及 Camoufox `home/` | `generated`：source 内容不复制；target 只按 transform manifest 重新建立明确登记的空根。source `.enterprise-platform.lock` 必须省略，target Platform 只能生成 `.agent-platform.lock`，不能在 target 数据根继续创建 source 锁名。P1 `data/.home/` 必须为空；Camoufox `home/` 只接受已审计的 `.cache/`、`.camoufox/`、`Downloads/`、`camoufox/` 顶层及三个指向镜像 `/opt/camofox/browser/` 的精确 symlink，且全树只能含部署用户拥有的普通目录、单链接普通文件和这三个 symlink |
