...
`ENTERPRISE_PLATFORM_DATA` 指定唯一状态根。仓库部署默认使用 `enterprise-agent-platform/data`。该目录不属于源代码，不能提交到 Git；受管上游 source 也只能位于该目录。
...
├── approvals/       # 按 scope + 稳定审批对象保存的持久 always 授权
...
`runtimes/cognee/` 保存按精确 revision 发布的 `source/<revision>/`、`.env`、data、system、cache 和 logs。`.env` 可能含外部 LLM、Embedding、数据库或对象存储凭据，属于 secret。Cognee 数据只是可选增强；本地知识文档仍以 `platform.db` 为权威。

## SearXNG 与 Firecrawl

`runtimes/searxng/` 保存平台生成的 Compose、settings、secret key、cache 和日志。`runtimes/firecrawl/` 保存按精确 revision 发布的 `source/<revision>/`、平台生成的 env、Compose override 和日志。容器 volume 由相应 Compose project 管理。

生成文件不得写回 `searxng` 镜像或 Firecrawl 受管 source。停止托管栈时必须执行平台记录的 Compose teardown。source 目录只由部署器持有的锁和 staging/rename 流程修改；Runtime 状态查询只读。
