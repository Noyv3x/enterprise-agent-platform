...
自更新与受控恢复的生产轮询节奏、watchdog 观测 deadline 和连续健康检查次数不受环境变量影响。只有使用 fake systemd 和本地 control socket 的包内单元测试可通过不序列化、不对外暴露的注入缩短轮询和 deadline；连续检查、身份校验、重试/重置与终态语义保持不变，真实 user-systemd 验收不使用该注入。
