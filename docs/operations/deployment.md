...
若任一候选 superseded pack 在检查时发生读取或 I/O 错误，Manager 必须保留当前 recovery pack、停在 `recovery_removed` 检查点之前并持久可重试诊断；运维验收必须观察到 `purged/completed` receipt，不得只根据部分删除位或对象暂时不可见宣布完成。源码 checkout 清单不得复用普通 Manager JSON 状态的 8 MiB 限制：退役、恢复和续传统一使用 256 MiB、两百万条记录的专用流式读取预算，并在触碰归档前验证每项路径、类型、mode、大小、摘要及 receipt 汇总。合法大清单必须能够自动重试通过，超出专用预算或结构异常的清单则保持 recovery pack 并返回可重试诊断。
