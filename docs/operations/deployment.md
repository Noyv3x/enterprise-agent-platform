...
若任一候选 superseded pack 在检查时发生读取或 I/O 错误，Manager 必须保留当前 recovery pack、停在 `recovery_removed` 检查点之前并持久可重试诊断；运维验收必须观察到 `purged/completed` receipt，不得只根据部分删除位或对象暂时不可见宣布完成。源码 checkout 清单不得复用普通 Manager JSON 状态的 8 MiB 限制：退役、恢复和续传统一使用 256 MiB、两百万条记录的专用流式读取预算，并在触碰归档前验证每项路径、类型、mode、大小、摘要及 receipt 汇总。合法大清单必须能够自动重试通过，超出专用预算或结构异常的清单则保持 recovery pack 并返回可重试诊断。受控故障恢复还可能在 `manager/recovery` 留下 `.download.<随机后缀>` 下载 staging；退役器只能删除 owner-only、非符号链接的匹配目录，目录最多包含 8 个顶层 owner-only 普通文件且总量不超过 512 MiB，文件名和内容类型必须属于固定 Manager ELF、SHA-256 校验文本及发布 manifest JSON 集合。空目录允许删除；任一未知文件、嵌套目录、符号链接、越权 mode、超限内容或伪造格式都必须保留整个 staging 并阻断检查，不得仅凭 `.download.` 前缀递归删除。
