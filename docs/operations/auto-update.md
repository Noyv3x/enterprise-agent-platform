...
A2 对该唯一 P1 前驱还执行一次窄、可重放且仅限 commit-release 的机器身份归一化。候选启动只能纯读接受 current marker/alias、canonical contract 中精确声明的 P1 历史形状，或二者逐 scope 混合；缺失 Runtime alias 只能从同一 scope 的唯一 current runtime row 确定性补齐。缺失、七字段容器 legacy 或六字段 logical legacy workspace marker 只有在 SQLite scope/runtime 行、规范相对 workspace、真实 owner-only 目录及 marker 已有字段全部一致时才可生成/升级；任何用户文件都不参与猜测。commit 以单一 SQLite 事务先补 alias，再逐个 fd-relative 原子发布 marker 与 sidecar；崩溃重放只接受相同 staging/current bytes，完成后启动与 evidence 必须只接受 current。P1 的 `app/home/memory/migration` 退役根不由 A2 删除，仍留在 source rollback 根并由桥接 transformer 按共享闭世界契约复验后省略。
