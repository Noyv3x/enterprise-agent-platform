...
handoff 返回后，旧进程不再清除维护标记或释放准入；从此状态所有权属于 updater。updater 获取仓库级 flock，记录旧 HEAD，执行 fetch、fast-forward 和完整 bootstrap。Cognee/Firecrawl source 不属于产品 Git 更新；bootstrap 按目标版本的上游源码契约准备它们。
...
`failed` 会继续阻断产品使用。运维应先保留 `$DATA/auto-update-state.json` 和部署日志用于诊断，检查 `git status --short`、当前/目标 revision 与 `$DATA/runtimes/*/source/` 中的受管源码状态，处理阻止安全回滚的本地变化，再重新运行 `./deploy.sh update` 由持有仓库锁的 updater 接管并恢复；不要手工删除或伪造维护 marker。若产品源码或旧版本受管 source 已无法安全部署，应从同一恢复点还原代码与数据后再重启服务。
