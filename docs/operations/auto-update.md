...
`release-transition.json` 当前使用 `target_baseline` 作为普通发布编排选择器，并把首个普通基线直接锚定到已封印的 Cleanup generation；它不表示部署仍在迁移，也不向产品二进制提供 handoff 能力。历史 C 候选的唯一直接前任是其版本中锁定的 Bridge generation：release assembler 当时可在 B 仍为 sealed draft 时读取并验证它，但 C 从一开始就是 schema 2、精确十镜像和中性 Compose/Manager 资产；source 工件、helper、handoff inventory 或任何一次性迁移附件均不得进入 C。公开 latest 仍为 A2 时 C 只能预构建为 sealed draft；统一 promotion evaluator 先凭 `source_owner_ready` 回执公开 B，再在收到绑定 B→C 的有效 `target_handoff_committed` 回执后公开 C。后续普通 release 只沿 target-only 基线前进。
...
首个 target-baseline 发布由 transition contract 精确锚定 Cleanup generation；Cleanup 尚未成为公开 latest 时，发布工作流必须在镜像构建前快速拒绝并等待，不能浪费一次完整多架构构建。该锚点只负责把 B/C 受控序列接入普通通道，不得被后续 generation 当作永久前任。转换编排退出后，普通 target-only 发布使用 main 通道级串行 release 队列，并在封印时把 promotion 的直接前任绑定为当时公开且已封印的 target-only latest。若当前 latest 已有一个密封的普通直接后继，后续发布等待该后继完成 promotion 后重新读取 latest；等待超时只失败当前尝试，不产生第二个同前任 draft。每次普通 promotion 完成后 evaluator 重新触发对最新已通过 Quality 的 main head 的 release 评估，使快速连续 push 最终自动追上 main；Bridge/Cleanup 固定 B/C 双 draft 与部署回执序列只作为已完成的封印审计图，不进入这条去抖路径。
