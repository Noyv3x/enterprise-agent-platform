...
release manifest 的镜像目录必须与 schema 对应的闭世界集合精确相等：schema 1 的 source-owner/Bridge 目录固定为十个运行镜像加一次性交接 helper，schema 2 的 Cleanup/target-only 目录固定为十个运行镜像且禁止 helper 与 `namespace_handoff`。JSON Schema、发布组装、Manager 解析器和静态验收共用这两套明确集合，不接受、保留或投影 opaque 额外镜像项；不存在按历史 generation、摘要或本地路径放宽解析的第三种兼容形状。schema 1 的部署证据必须校验全部十一个不可变 digest，但只把九个固定 Compose 服务视为常驻容器；`agent-sandbox` 通过登记表单独对账，`handoff-fs-helper` 不得作为 Compose 服务出现。
