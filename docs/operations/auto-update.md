...
A2 首次 promotion 的 current 是唯一早于封印协议的公开 P1 `983f79b4900502f35fac6de8154eb344fc9f143b`。evaluator 只能在 `source_owner_compat` 同时绑定该 generation、真实 `release.json` 摘要 `8772fc457552c48cb5c9623b4411647e78dde18065df07d6520ac6b9d32520c1` 与 Compose 摘要 `ebe1ce922cd33c9acb816bf9af175fc7e3838835cb413ab3ee91b91808698954` 时，接受它真实存在的八资产旧 release：资产必须逐项与 GitHub API ID/digest/size 相符，manifest 必须是精确十镜像普通闭世界。该 release 本来没有 `promotion.json` 或 Actions provenance，不能反向要求它具备新资产；除此精确 generation/bytes 之外，没有任何“旧 release”兼容入口。首个 source-owner 成为 latest 后，所有后继都必须走完整封印和出处证明。

Bridge 尚未公开时，若已经公开的 source-owner 仍缺少使唯一 P1 安全升级所需的能力，只允许用一个同阶段、普通 manifest 的 source-owner 稳定化后继修复；不能改写、删除、替换或降级已经公开的 generation。稳定化候选的 `predecessor_generation` 必须精确等于当前 latest，当前 release 也必须是同一 transition/source/target profile 的 `source_owner`，候选仍需完整封印、出处证明与匿名镜像复验，且不使用部署回执。`source_owner_compat` 始终只钉死上述唯一 P1 generation/bytes，它是部署机可从 P1 直接跳到最终 source-owner 的运行时兼容证据，不是稳定化候选的发布图前任；不得随 `predecessor_generation` 泛化或改绑。Bridge B 只能以前述最终、已在部署机稳定运行的 source-owner 为直接前任，旧 source-owner 的回执不能授权后继 Bridge。
...
技术切换前后使用四个连续且可独立验收的逻辑阶段。第一个普通 source-profile 发布只集中现役 source identity、固定签名目标 profile ID、加入严格的 `namespace_handoff` 清单解析，并在保存 generation 工件、下载发布工件、写入 Candidate/Activation 或进入维护之前拒绝该描述符；`Check` 只返回确定性错误，经 operation 触发的更新则可以且必须保留有界失败审计，但两者都不产生交接副作用，也不携带未接线的 journal、listener 或 helper 原语。第二个普通 source-owner 阶段 A2 一次性交付并接通完整 coordinator、持久 helper、journal、listener 交接、恢复和启动发现路径，但自身 release manifest 仍使用普通闭世界形状且明确不携带 `namespace_handoff`，因此只能在源身份下稳定运行和接受断电矩阵验收；Bridge 公开前允许按上一段规则增加直接的同阶段稳定化 generation，但不能跳过部署验收。只有唯一现役部署用其部署级 Ed25519 密钥对一次性 challenge 生成针对最新 source-owner 的 `source_owner_ready` 回执，且统一 promotion evaluator 验证它正在空闲 A2 source-owner capability 上运行后，第三个不可变 draft B 才可公开并成为 latest。第四个不可变 draft C 只在同一部署签发 `target_handoff_committed` 回执，证明 B 已 `committed`、`target_ack` 与源退役证据完整，且目标 Manager 在退役后成功完成一次认证通道检查后才可公开。这样现役 Manager 不需要猜测未知字段，桥接 Candidate 也不会在普通 activation/watchdog 尚未结算时搬移自己的状态根；任一中间发布失败都保留原源身份运行。描述符必须同时提供现役 Manager 可读取的源 Compose/Manager 工件和目标 Manager、目标 Compose 工件，所有 URL、摘要、源/目标 identity 和 bridge generation 都属于同一不可变清单；缺少任一目标工件时只能把该发布视为不可执行，不能退回普通 update。
...
只有一个在 `main` 全局锁下运行的 promotion evaluator。它对普通与转换发布共用同一个当前 latest、直接前任和不可变工件判定：首个 A2 的直接前任是 source-profile 公开 generation；Bridge 前如有 source-owner 稳定化发布，只能逐个直接跟随当前 source-owner；B 的直接前任是最终 source-owner，C 的直接前任是 B。任一步都不得跳过、用更新后代替换或从另一个 workflow 改写 latest。A2 与其稳定化后继保持普通 manifest，其转换角色与前任由 evaluator 的已锁定发布计划绑定，不得为此向 manifest 加描述符。B/C draft 在完整资产目录与摘要封印后禁止 `--clobber`、删除或替换资产；同 tag 的任何字节差异都是 immutable-ID collision。公开前同名 Git ref 必须已是直接指向 candidate commit 的 lightweight commit ref；不存在时只允许受信 evaluator 用 create-only API 建立并立即复验，错误 ref、annotated tag、传输不确定或竞争都在 visibility 变更前失败关闭。构建成功、后代 commit 更新或普通“最新合格发布”逻辑都不能自动把 B/C 变为 non-draft；只有收到当次 challenge 的有效回执后，该唯一 evaluator 才能显式公开对应的直接后继。候选的全部十一个镜像 digest 在 visibility 变更紧邻前后都必须以无凭据 registry 请求复验；后验发现不可达时要明确报告“发布已可见”的高危事故，不得声称 workflow 失败已回滚 visibility。
...
目录首次 rename 和 exact-final 崩溃重放使用同一耐久屏障：先 `fsync` 已固定的 child fd，再无条件 `fsync` staging/source parent（即使 staging 名已不存在），最后以 `fsync` destination parent 作为提交点；清理 residue 后仍须重新执行完整屏障。任一步失败都必须在复验 final 仍为预期 inode 后返回携带该 identity 的 committed error，下一次 retry 必须再次执行完整屏障，不能仅因 final 已存在便释放 reservation。
