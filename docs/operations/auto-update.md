...
早于封印协议的首次 promotion 已经完成，当前公开直接前任具备完整 `promotion.json` 与 Actions provenance。Bridge 阶段及其后续 evaluator 当时只接受完整封印的 current release；历史 P1 的特殊资产形状、摘要夹具、`source_owner_compat` 解析器和 `--current-compat-root` 入口全部退出运行路径，不再作为新基线的一部分。当前普通候选必须从完整封印的 target-only 直接前任单调推进。

同一次已完成升级使用的无 `--config` watchdog 启动例外和 `/v1/status.workspace_schema_commit` 一次性授权也随之退役。新基线的普通 watchdog 始终显式绑定已路由配置，Manager status 不再投影 workspace 迁移能力；Bridge transformer 当时验证的已登记 source 库存只属于历史输入证据，不得重新开启普通更新兼容入口。

历史 Bridge B 候选只允许以 canonical contract 固定的、当时已在部署机稳定运行且完整封印的最终 source-owner 为直接前任。该阶段发布图不接受新的 source-owner 候选或同阶段稳定化分支；历史、重复或绑定其它 challenge 的 source-owner 回执不能授权 B，只有同时绑定该固定直接前任、B generation 与当次一次性 challenge 的新鲜 `source_owner_ready` 回执有效。
...
release manifest 的镜像目录必须与阶段 schema 对应的闭世界集合精确相等：历史 schema 1 source-owner/Bridge 目录固定为十个运行镜像加一次性交接 helper；当前 schema 2 Cleanup/target-only 目录固定为十个运行镜像且禁止 helper 与 `namespace_handoff`。发布编排仍可复验历史封印，但当前安装器和 Manager 只解析 schema 2；不存在按历史 generation、摘要或本地路径放宽产品解析的兼容形状。

发布器、匿名镜像复验和资产封印按 canonical transition stage 复验唯一协议。历史 Bridge 候选只允许 schema/protocol 1、十一镜像和 helper，并为 source 发现临时使用 `ubitech-compose.yaml` 与 `ubitech-manager-linux-{amd64,arm64}`；Cleanup 与当前 target baseline 只允许 schema/protocol 2、十镜像，资产名固定为中性的 `agent-platform-compose.yaml` 与 `agent-platform-manager-linux-{amd64,arm64}`，且不得构建、上传或引用 helper。阶段 Manager 资产都带同 basename 的 `.sha256` sidecar，封印目录必须按阶段精确选择，不接受混合名称；当前安装器只选择中性 schema 2 资产。schema 2 的安装器与运行中 Manager 必须执行同一工件闭集：Manager version 精确等于 `source_commit`、架构键精确为 `amd64`/`arm64`、每个 URL basename 与中性资产名相等，Compose basename 精确为 `agent-platform-compose.yaml`，URL 不含凭据、query 或 fragment；Manager 下载并校验候选字节后还必须执行候选的只读 `version` 命令，并要求其输出逐字等于清单 version，不能只接受任意非空输出。不能出现“fresh install 会拒绝、自动更新却接受”的较弱解析器。历史 Cleanup C 候选在 B 仍是已封印 draft、公开 latest 仍是最终 source-owner 时预构建：构建端通过 canonical `predecessor_generation` 从认证的完整 release 列表中按 `tag_name` 唯一解析 `container-<B>` draft，再通过该 release 对象返回的精确 asset ID 逐字节复验其 seal、provenance 与工件；不能依赖只暴露已发布 release 的 tag-name REST endpoint，也不能把 public latest 冒充 C 的直接前任。promotion 时重新证明 B 已公开、C 是其唯一直接后继并具备有效 target 回执。普通 target-baseline 只跟随当前公开 target-only latest。

`release-transition.json` 当前保留 `cleanup` 作为发布编排和封印审计选择器，不表示部署仍在迁移，也不向产品二进制提供 handoff 能力。历史 C 候选的唯一直接前任是其中锁定的 Bridge generation：release assembler 当时可在 B 仍为 sealed draft 时读取并验证它，但 C 从一开始就是 schema 2、精确十镜像和中性 Compose/Manager 资产；source 工件、helper、handoff inventory 或任何一次性迁移附件均不得进入 C。公开 latest 仍为 A2 时 C 只能预构建为 sealed draft；统一 promotion evaluator 先凭 `source_owner_ready` 回执公开 B，再在收到绑定 B→C 的有效 `target_handoff_committed` 回执后公开 C。这一序列已经完成，当前实例和普通 release 只沿 target-only 基线前进。

Cleanup 与 target-baseline 的质量门和 release 组装门还必须运行同一独立的 target-only source-tree gate。该门从 canonical transition contract 读取 stage：Bridge 明确跳过；另两个 stage 对产品实际编译、打包和容器入口的闭世界路径集合执行扫描，要求 source profile 常量、旧技术环境变量/路径/资产名、一次性 Manager 命令、namespace handoff parser、helper/coordinator/transformer 模块和生成的 P1 inventory 已从生产树物理删除。至少必须逐字拒绝 `ENTERPRISE_`、`SOURCE_TECHNICAL_PROFILE`、`enterprise_session` 与 `.enterprise-platform.lock` 这些遗留 source 标量；它们不能因为没有 `UBITECH_` 前缀而漏过。扫描不遍历 docs、release orchestration 或第三方依赖；历史值只允许保留在 `_test`、`tests`、`testdata` 和 `scripts/tests/fixtures` 这些非产品夹具边界，数据库演进代码可以保留与用户数据有关的表/字段名，但不得借迁移名义保留任何 source technical-profile 选择器或可执行兼容入口。扫描根缺失、未知文件类型、符号链接、不可解码文本或任一命中都失败关闭；不能用 allowlist 参数在 workflow 中放宽。

普通 target-baseline 发布使用 main 通道级串行 release 队列，并在封印时把 promotion 的直接前任绑定为当时公开且已封印的 target-only latest，而不是把过渡 contract 的锚点冒充每一代永久前任。若当前 latest 已有一个密封的普通直接后继，后续发布等待该后继完成 promotion 后重新读取 latest；等待超时只失败当前尝试，不产生第二个同前任 draft。每次普通 promotion 完成后 evaluator 重新触发对最新已通过 Quality 的 main head 的 release 评估，使快速连续 push 最终自动追上 main；Bridge/Cleanup 固定 B/C 双 draft 与部署回执序列只作为已完成的封印审计图，不进入这条去抖路径。
...
### 已完成的 Bridge→Cleanup 发布编排与封印审计

source-profile、完整 source-owner A2、Bridge B 与 Cleanup C 已经发布并完成验收，当前不存在活动转换。本节保留当时两发布事务的封印、恢复和供应链审计规则，不描述现役 Manager 的可执行职责。阶段 B 执行[技术命名空间交接](deployment.md#技术命名空间交接)，阶段 C 随即形成 target-only 基线；A2 保持不携带 `namespace_handoff` 的普通 manifest，B/C 候选则以不可变 sealed draft 产出，不能伪装为普通镜像 generation 更新。Bridge 候选当时提供 source-owner Manager 可发现和校验的原技术资产，同时把签名目标固定为 `agent-platform-manager`、`agent-platform-manager.service`、`~/.config/agent-platform`、`~/.local/share/agent-platform`、`/var/lib/agent-platform`、`agent-platform`、`agent-platform_core`、`AGENT_PLATFORM_*`、`io.agent-platform.*`、`agent-platform-sandbox-*` 和 `.agent-platform`。它在进入维护前必须证明系统处于无 operation、无 activation/watchdog、无执行调用的稳定边界，并先持久化绑定源/目标全部身份的 handoff journal；只有目标 Manager 接管、目标核心 generation 健康、自动更新轮询可达且 source 身份安全退役后才能 finalize。

阶段 B 的顶层 `manager`、`compose` 与 `images` 是完整 target generation，`namespace_handoff.target` 必须与顶层 Manager/Compose 逐字段一致；`namespace_handoff.source` 则逐字段复用直接前任 source-owner 的公开 manifest，不能把 B 的 target Compose 同时冒充源恢复工件。为兼容当时的 source-owner 发现，B release 暂时保留 source 时代的资产文件名，但文件名不决定技术身份：其中 Compose 正文只消费 `AGENT_PLATFORM_*`，只挂载 target data/secret/control 根并使用 target project、network、label、Sandbox 和 marker。Bridge 镜像入口以闭世界 technical-profile 绑定消费同一 target 根，未知或混合 profile 在启动 writer 前失败关闭。B 公开到 C 公开之间不接受全新安装；阶段 C 与后续 target baseline 从编译期 target-only stage 选择 Manager/config/data/unit/socket。当前安装器只接受中性 schema 2 target 工件和路径，不能根据 journal、basename 或环境回到 source profile。

Bridge 阶段失败必须由同一 journal 回到唯一 source Manager 和原业务入口；目标状态不完整、两套 unit 可能启动、路径或 Docker ownership 无法证明、持久摘要变化时均保持维护并失败关闭。阶段 C 只在 bridge terminal commit 后移除一次性识别与迁移实现，普通发布不得永久同时接受两套路径、unit、label、Cookie、API 或 release asset。CI 当时从上一公开基线真实执行 Bridge 并在持久阶段注入终止，证明只存在一个更新所有者，再验证 Cleanup 无法重新识别 source 命名空间；main 通道只有在部署写入 `target_ack`、提交 `committed`、完成 source 退役并通过 target Manager 认证检查后才把 latest 推进到 C。

#### 历史四阶段的触发与所有权

技术切换使用了四个连续且可独立验收的逻辑阶段。第一阶段 source-profile 完成 source identity 集中与无副作用描述符拒绝；第二阶段 source-owner A2 接通 coordinator、持久 helper、journal、listener 交接、恢复和启动发现路径，但普通 release manifest 仍明确不携带 `namespace_handoff`，且最终 A2 不接受同阶段后继。唯一部署随后对一次性 challenge 签发绑定 A2→B 的 `source_owner_ready` 回执，evaluator 验证空闲 A2 capability 后才公开不可变 B draft；同一部署再签发 `target_handoff_committed` 回执，证明 B 已 `committed`、`target_ack` 与 source 退役证据完整，target Manager 也完成认证通道检查后才公开 C。B 描述符同时提供当时 source-owner 可读的 source Compose/Manager 与目标工件；任一工件缺失都使候选不可执行，不能退回普通 update。

阶段发布由 `main` 全局锁下唯一 promotion evaluator 控制。它对普通与转换发布共用同一个 latest、直接前任和不可变工件判定：B 候选的直接前任固定为最终 source-owner，C 候选的直接前任固定为 B，任一步不得跳过或由其它 workflow 改写。B/C draft 封印后禁止覆盖、删除或替换资产；只有收到当次 challenge 的有效回执后 evaluator 才能公开精确直接后继。候选 manifest 的全部镜像 digest 在 visibility 变更紧邻前后都以无凭据 registry 请求复验；后验不可达属于“发布已可见”的高危事故，不能声称 workflow 失败已回滚 visibility。

阶段转换发布不依赖仓库外的 predecessor black-box harness、固定十六重启矩阵或独立 qualification artifact。Bridge 发布证据由仓库内确定性 phase/recovery 测试、真实 user-systemd `SIGKILL` 集成门、消费同一已验证镜像目录的 Compose 实栈冒烟，以及唯一部署机签署的 challenge-bound 回执共同构成。确定性测试逐阶段验证 journal/state 双文件边界、listener 转交、单一 Manager/listener/Platform writer、forward-only commit 和原子 source rollback；systemd 门证明持久 helper/Manager 被真实 unit 恢复且没有并行旧进程；Compose 门验证 target Manager、Compose、镜像、持久数据和运行身份；部署回执把候选、直接前任、实际运行 Manager 摘要与权威 handoff 状态绑定。publisher 的普通 Actions provenance 闭世界绑定当次 source commit、Quality run 和 sealed release 资产，promotion evaluator 重证这些身份；它不能用日志文字或手填布尔值替代任一门。唯一生产部署没有执行破坏性故障注入，只通过 owner-only CLI 对当次 challenge 产生 `source_owner_ready` 或 `target_handoff_committed` 签名回执。
...
#### Bridge 历史实现切点与发布门（非当前职责）

以下清单记录 Bridge 阶段当时必须同时接通的实现边界，用于审计已完成的 source-owner→Bridge→Cleanup 发布序列。Cleanup/target-baseline 的当前 Manager 不再编译、注册或运行这些 helper、handoff、source 路由和观察入口；这些条目不能被解释为现役产品职责：
...
历史 Cleanup generation 只有在验收得到可连续执行普通 D/E 更新的 `target_baseline` 后才算完成：下一次普通更新必须只消费 target profile 的 manifest、Compose、Manager、数据根与 Docker ownership，不能继续等待桥接 descriptor、source reader 或一次性 helper；否则当时的更新通道仍视为冻结，不能删除最后恢复证据或报告清理成功。

在这些切点尚未全部实现并通过端到端崩溃矩阵时，历史 release 只能作为源侧能力的前置基线，不能发布 `namespace_handoff` 描述符，也不能暴露一个表面成功的 handoff 命令。当时普通 operation journal、自更新 plan、配置默认值、Compose environment、Docker ownership 和 Sandbox registry 均绑定 source 身份；直接启用交接会使恢复 watchdog 找不到原路径、目标 Manager 无法证明 Current，或让两套控制面竞争同一端口和 Docker 对象。这段约束只解释 Bridge 发布门的历史安全理由，不授权当前二进制恢复任何对应入口。
...
当前 target-only 宿主 CLI 的手动 `update` 必须通过认证 control socket 调用一次 `check`，并且只接受普通 schema 2 manifest，再按普通 operation 流程提交和等待 update。`namespace_handoff`、source 描述符或其它 transition 字段在 manifest 闭世界解析时直接拒绝；CLI 不包含 handoff 分流、source 路由、helper 或 transition journal 入口。未知子命令必须在读取 Manager 配置或状态前由当前静态命令表拒绝。

Bridge 历史阶段曾要求 `check` 在普通 manifest 与 `namespace_handoff` manifest 之间分流：后者由服务端建立或复用独立 handoff，CLI 随即停止且不向 `/v1/operations` 提交普通 update。该行为只作为已完成发布阶段的审计证据保留，不属于 Cleanup/target-baseline 的可执行契约。
...
普通 rollback 的 plan-first 半 checkpoint 只有在 Candidate 的 version、source commit、SHA-256、验证时间和 `platform_committed=true` 均完整，Candidate 路径精确等于其受管 version 目录中的 target-profile basename `agent-platform-manager`，Activation 的 plan path 精确等于该 Candidate 的受管 plan 路径，且 plan/state/stable/运行 inode 全链一致时才可由启动流程补清。`pathWithin`、目录名前缀或单独 hash 命中都不足以建立该终态所有权；任一字段篡改必须保持 state 不变并失败关闭。
...
当前 target-only watchdog 与 `recover-current` 可能从 stable 之外的受管不可变 inode 运行，但都必须显式携带规范绝对 config，并从同一个当前 UID 控制、非符号链接的配置 inode/字节快照构造 target RuntimePaths。watchdog 由 activation plan、`recover-current` 由 recovery journal 分别绑定 target stable 路径、候选/恢复路径、运行 inode、SHA-256、unit、Platform commit 和事务；命令行 config 只能与该持久绑定逐字节一致，不能选择另一套身份。它们不读取 handoff journal，也不从 source 路径、环境、basename 或历史摘要推断 profile。普通 Manager 与宿主 CLI 仍要求运行中 inode和 target stable inode 精确一致。

普通自更新启动的独立 `self-update-watchdog` 可能与仍持有同一全局 handoff observation 的当前 Manager 短暂重叠：当前 Manager 必须先证明 watchdog 已由 systemd 接管，随后才会退出该普通 operation 并释放 observation。因此只允许这个 watchdog 的中立 technical routing 在 `handoff.ErrBusy` 上执行有界、可取消且有间隔的等待；一旦 observation 释放，watchdog 必须重新从磁盘完成完整 authority 路由和后续 plan 身份验证。等待耗尽、调用 context 取消或 deadline 到期均失败关闭，且不得使用等待前的身份快照。其它路由错误不重试；`recover-current`、普通 CLI、`serve`、helper 与其它入口遇到相同 busy 或任意路由错误仍立即失败，不能把这项启动交叠例外扩展为通用锁等待或新的迁移兼容层。
...
- 受控恢复在 unit fence、stable 替换、intent、watchdog ownership 交接、重新启用主 unit 和 terminal journal 的每个持久边界重启后均只能继续同一事务；
